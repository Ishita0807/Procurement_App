...
import { AlertCircle, ArrowLeft, FileSpreadsheet } from "lucide-react";
...
export default function UploadPage() {
  ...
  const processUploadedFile = async () => {
    ...
    try {
      setError(null);
      setProcessing(true); // ⬅️ move to the start
      ...
      // Get supplierFileId directly from upload response
      const supplierFileId = uploadRes.data.supplierFile.id;

      // Process file
      const processRes = await axiosInstance.post("/process-file", {
        supplierFileId,
      });

      console.log("Process:", processRes.data);

      const supplierArray: Supplier[] = processRes.data.output as Supplier[];
      console.log(supplierArray, supplierArray.length)
      // Calculate scores 
      const suppliersWithScores = supplierArray.map((s) =>
        calculateSustainabilityScore(s)
      );

      // Sort and rank
      suppliersWithScores.sort((a, b) => (b.score || 0) - (a.score || 0));
      suppliersWithScores.forEach((s, idx) => (s.rank = idx + 1));
      ...
      setError(
        err?.response?.data?.error || err.message || "Error processing file"
      );
    } finally {
      setProcessing(false); // ⬅️ always reset, even on error
    }
  };
  ...
  const saveSuppliers = async () => {
    ...
    try {
      ...
      if (res.status !== 200) throw new Error("Failed to save suppliers");
